...
export default async function EditLessonPage({
  params,
  searchParams,
}: PageProps<"/[locale]/admin/lessons/[id]">) {
    ...
    <div className="max-w-2xl pb-16">
      ...
      <div className="mt-10 border-t border-border pt-6">
          ...
          {(questions ?? []).map((q) => (
            <div key={q.id} className="rounded-lg border border-border bg-surface p-4">
              <div className="flex items-start justify-between gap-4">
                <div>
                  <p className="text-sm font-medium">{q.question.en}</p>
                  <p className="text-xs text-muted">{q.question.de}</p>
                </div>
        ...
        >
          ...
          <input
            name="question_de"
            placeholder="Question text (German)"
            required
            className="w-full rounded-lg border border-border bg-surface-2 px-3 py-2 text-sm"
          />
          {[0, 1, 2, 3].map((i) => (
